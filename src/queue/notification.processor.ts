...
export class NotificationProcessor extends WorkerHost {
  ...
  async process(job: Job<NotificationJobData>): Promise<any> {
    ...
    this.logger.log(`Processing notification ${transactionId} via ${channel}`);
    ...
    try {
      ...
      this.logger.log(`Notification ${transactionId} sent successfully`);
      return result;
    } catch (error) {
      this.logger.error(
        `Failed to send notification ${transactionId}: ${error.message}`,
        error.stack,
      );
